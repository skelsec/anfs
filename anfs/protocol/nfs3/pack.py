...
def nextiter(d):
	if isinstance(d, list):
		d = d[0]
	nextentry = None
	if 'nextentry' in d and len(d['nextentry']) > 0:
		nextentry = d['nextentry'][0]
		del d['nextentry']
	yield d
	if nextentry is not None:
		for i in nextiter(nextentry):
			yield i
...
class nfs_pro_v3Unpacker(Unpacker):
	...
	def unpack_nfstime3(self, data_format='json'):
		...
		total_seconds = data.seconds + (data.nseconds / 1_000_000_000.0)
	
		# Convert the total seconds to a datetime object
		res = datetime.datetime.fromtimestamp(total_seconds)
		return res
...
class NFSFileEntry:
	...
	def from_attrs(attrs):
		res = NFSFileEntry()
		res.fsid = attrs.get("fsid")
		res.atime = attrs.get("atime")
		res.mtime = attrs.get("mtime")
		res.ctime = attrs.get("ctime")
		res.fileid = attrs.get("fileid")		
		res.type = attrs.get("type")
		res.mode = attrs.get("mode")
		res.nlink = attrs.get("nlink")
		res.uid = attrs.get("uid")
		res.gid = attrs.get("gid")
		res.size = attrs.get("size")
		res.used = attrs.get("used")
		return res
	
	@staticmethod
	def from_dict(data, encoding='utf-8'):
		realhandle = None
		try:
		...
		res.name = res.name.decode(encoding)
		res.fileid = data.get("fileid")
		na = data.get("name_attributes")
		if na is None:
			return res, realhandle
		attributes = na.get("attributes")
		if attributes is None:
			return res, realhandle
		res.parent = attributes.get("parent")
		res.atime = attributes.get("atime")
		res.mtime = attributes.get("mtime")
		res.ctime = attributes.get("ctime")
		res.size = attributes.get("size")
		res.type = attributes.get("type")
		res.uid = attributes.get("uid")
		res.nlink = attributes.get("nlink")
		res.gid = attributes.get("gid")
		res.mode = attributes.get("mode")
		res.fsid = attributes.get("fsid")
		return res, realhandle
	
	def __str__(self):
		return f"{self.name} {self.fileid} {self.type} {self.size} {self.uid} {self.gid} {self.mode} {self.fsid} {self.atime} {self.mtime} {self.ctime}"

	def to_line(self):
		t = [
			file_mode_to_string(self.type, self.mode),
			self.nlink,
			self.uid,
			self.gid,
			self.size,
			format_datetime_like_ls(self.ctime),
		]
		if self.symtarget is not None:
			t.append('%s -> %s' % (self.name, self.symtarget))
		else:
			t.append(self.name)
...
def format_datetime_like_ls(dt):
	...
	six_months_ago = now - datetime.timedelta(days=182.5)  # Approximation
	six_months_ahead = now + datetime.timedelta(days=182.5)  # To handle future dates similarly
	if six_months_ago <= dt <= six_months_ahead:
		# File modified within the last six months or in the near future
		format_str = "%b %d %H:%M"
	else:
		# File modified more than six months ago or in the distant future
		format_str = "%b %d  %Y"
	return dt.strftime(format_str)

def file_mode_to_string(ftype, mode):
	if mode is None:
		return ''
	# File type
	if ftype == 2:  # directory
		...
		type_char = '-'
	elif ftype == 5:  # symbolic link
		type_char = 'l'
	else:
		type_char = '?'  # default to regular file for simplicity
	# Permissions
	permissions = ''
	for i in range(3):
		# Shift mode to the right to get to the relevant bits
		shifted_mode = mode >> (6 - i*3)
		permissions += 'r' if shifted_mode & 0o4 else '-'
		permissions += 'w' if shifted_mode & 0o2 else '-'
		permissions += 'x' if shifted_mode & 0o1 else '-'
